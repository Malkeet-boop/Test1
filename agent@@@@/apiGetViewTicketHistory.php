...
$qry="Select ts.id,ts.ticket_date,ts.subject,vsm.verification_status from tw_ticket_system ts INNER JOIN tw_verification_status_master vsm ON ts.status=vsm.id where ts.status='".$settingValueCompletedStatus."' and ts.sender_id='".$agent_id."' and ts.panel='Agent'";

$qry1="select count(*) as cnt from tw_ticket_system where status='".$settingValueCompletedStatus."' and sender_id='".$agent_id."' and panel='Agent'";
